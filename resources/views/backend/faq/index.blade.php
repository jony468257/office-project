                ...
                <ol class="inline-flex items-center space-x-1 ltr:md:space-x-3 rtl:md:space-x-0">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="#" class="ltr:ml-1 rtl:mr-1 text-sm font-medium text-gray-500 hover:text-gray-900 ltr:md:ml-2 rtl:md:mr-2 dark:text-gray-100 dark:hover:text-white">{{ __('messages.faq') }}</a>
                        </div>
                    </li>
                </ol>
            </nav>
        ...
        <div class="col-span-12">
            <div class="card dark:bg-zinc-800 dark:border-zinc-600">
                <div class="card-body pb-0">
                    <h6 class="mb-1 text-2xl text-gray-700 dark:text-gray-100">{{ __('messages.faq') }} {{ __('messages.list') }}</h6>
                        ...
                        @foreach ($datas as $key => $data)
                            @foreach ($data->faq_items as $faq)
                                <tr>
                                    ...
                                    <td class="p-4 pr-8 border border-t-0 border-l-0 border-gray-50 dark:border-zinc-600">{{ $faq['faq_question'] }}</td>
                                    <td class="p-4 pr-8 border border-t-0 border-l-0 border-gray-50 dark:border-zinc-600">{{ $faq['faq_answer'] }}</td>
                                    <td class="border rtl:border-l border-t-0 border-l-0 border-gray-50 dark:border-zinc-600">
                                        <div class="gap-1 text-center flex justify-center">
                                            <button type="button" class="btn text-white bg-yellow-500 border-yellow-500 hover:bg-yellow-600" title="{{ __('messages.edit') }}" onclick="window.location.href='{{ route('faq.edit', $data->id) }}'">
                                                <i class="bx bx-pencil text-16 align-middle"></i>
                                            </button>
                                            <form action="{{ route('faq.destroy', ['id' => $data->id, 'faqIndex' => $loop->index]) }}" method="POST" class="ml-2">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn text-white bg-red-500 border-red-500 hover:bg-red-600" title="{{ __('messages.delete') }}">
