        ...
        <div class="col-span-12">
            <div class="card dark:bg-zinc-800 dark:border-zinc-600">
                <div class="card-body pb-0">
                    <h6 class="mb-1 text-15 text-gray-700 dark:text-gray-100">{{ __('messages.slider') }} {{ __('messages.list') }}</h6>
                        ...
                        @foreach($datas as $key => $data)
                            <tr>
                                ...
                                <td class="p-4 pr-8 border rtl:border-l border-t-0 border-l-0 border-gray-50 dark:border-zinc-600 text-center">
                                    <img class="inline-block h-14 w-50 rounded ring-2 ring-white dark:ring-zinc-500" src="{{ asset('storage/').'/'. $data->image }}"  alt="{{ $data->image }}"/>
                                </td>
                                <td class="border rtl:border-l border-t-0 border-l-0 border-gray-50 dark:border-zinc-600">
                                    <div class="gap-1 text-center flex justify-center">
                                         <button type="button" class="btn text-white bg-yellow-500 border-yellow-500 hover:bg-yellow-600 hover:border-yellow-600 focus:bg-yellow-600 focus:border-yellow-600 focus:ring focus:ring-yellow-500/30 active:bg-yellow-600 active:border-yellow-600" title="{{ __('messages.edit') }}" onclick="window.location.href='{{ route('slider.edit', $data->id) }}'"><i class="bx bx-pencil text-16 align-middle "></i></button>
                                        <div>

                                        <form action="{{ route('slider.destroy', $data->id) }}" method="POST" onsubmit="return confirmDeletion()">
                                            @csrf
                                            @method('DELETE')
